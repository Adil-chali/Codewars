...
function knapsack(capacity, items) {
  ...
  const extandedItems = items.map(([size, value], i) => {
      const ratio = value / size;
      return [size, value, ratio, i];
    });
  const sortedValueRatio=extandedItems.sort((a,b)=>b[2]-a[2])
  for(let i=0; i<sortedValueRatio.length;i++){
    let index=sortedValueRatio[i][3]
    let itemCap=sortedValueRatio[i][0]
    if(itemCap>remainingCap){
      result[index]=0
    }else{
      result[index]=Math.floor(remainingCap/itemCap)
      remainingCap=remainingCap-itemCap*result[index]
    }
  }
  return result
}