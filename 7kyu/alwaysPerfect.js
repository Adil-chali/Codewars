...
function checkRoot(string){
 const splitStr=string.split(",")
   if (splitStr.length !== 4 || splitStr.some(x => isNaN(x))) {
    return 'incorrect input';
  }
  const nums = splitStr.map(Number);
  if (nums.some((x, i) => i < 3 && nums[i+1] - x !== 1)) {
    return 'not consecutive';
  }
const productOfFourPlusOne=  splitStr.reduce((acc,cur)=>acc*cur,1)+1
if(splitStr.length !== 4 ||splitStr.some(x=>x===isNaN(x))){
  return 'incorrect input';
}
const squareRoot =Math.sqrt(productOfFourPlusOne)
return `${productOfFourPlusOne}, ${squareRoot}`
}